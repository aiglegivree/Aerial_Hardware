# -*- coding: utf-8 -*-
"""
drone_controller_triangulation_center.py
========================================

Same scaffolding and state machine as drone_controller_triangulation.py, but
triangulation uses only the *centre* of the gate quad rather than all 4
corners. We cast one ray per view through the centroid of the detected quad,
intersect the two rays in 3-D (least-squares), and use that point as the gate
centre.

Because a single ray gives no information about the gate's plane orientation,
the gate yaw is approximated as the bearing from the recovered gate centre
back toward the midpoint of the two observation positions — i.e. we assume
the gate faces the drone. The approach/exit waypoints are placed along that
bearing.
...
# Skew (closest-approach distance between the two rays) above which we reject
# the triangulation as inconsistent. The two rays should intersect; a large
# skew means we're looking at different gates or one view is bad.
TRIANGULATE_SKEW_MAX = 0.30   # m

# Range gate centre must fall within (relative to drone) — sanity bound.
GATE_RANGE_MIN = 0.2
GATE_RANGE_MAX = 6.0
...
class GateController:
    ...
    def __init__(self, cf: Crazyflie, cam: UdpVideoThread = None):
        ...
        # Two-view data — just one ray per view through the gate centre.
        self._P = None              # view-1 camera position (world)
        self._r_centre = None       # view-1 centre ray (world frame, unit-ish)
        self._Q = None              # view-2 camera position (world)
        self._s_centre = None       # view-2 centre ray (world frame, unit-ish)
        self._qn1 = None            # view-1 quad_norm (kept for size sanity check)
        self._qn2 = None            # view-2 quad_norm
    ...
    def _centre_ray(self, quad_norm, s):
        """One world-frame ray through the gate centroid."""
    ...
    def _triangulate(self):
        ...
        Returns (H (3,), skew_m). Raises ValueError if the rays are nearly
        parallel or the result is geometrically implausible.
        """
        r = self._r_centre
    ...
    def _set_gate_waypoints(self, H, s):
        """With centre-only triangulation we have no gate-plane normal.
        Approximate the gate yaw as the bearing from the gate back toward the
        midpoint of the two observation positions — i.e. assume the gate
        faces the drone. Approach/exit lie along that axis.
        """
        mid = 0.5 * (self._P + self._Q)
        dx = mid[0] - H[0]
        dy = mid[1] - H[1]
        ...
        # gate_yaw: direction the gate's forward (pass-through) axis points,
        # which is FROM the observers TOWARD the far side, so we negate.
