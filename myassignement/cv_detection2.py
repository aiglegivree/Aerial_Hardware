...
def split_with_watershed(mask):
    """
    Splits touching/merged blobs (e.g., overlapping gates) into separate regions.
    Returns a new binary mask where objects are separated.
    """
    # ensure binary
    ...
    # find sure peaks (object centers)
    _, sure_fg = cv2.threshold(dist, 0.4 * dist.max(), 255, 0)
    sure_fg = np.uint8(sure_fg)

    # unknown region
    sure_bg = cv2.dilate(bin_img, np.ones((3,3), np.uint8), iterations=2)
    ...
    ws_input = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    markers = cv2.watershed(ws_input, markers)

    # output separated mask
...
def detect_gate(gray, k=K, dist_coeffs=DIST,
    ...
    mask = split_with_watershed(prep['filled'])

    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    #contour = max(accepted, key=lambda x: x[1] * 0.7 + cv2.boundingRect(x[0])[2])[0]
