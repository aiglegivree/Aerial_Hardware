...
# If SEARCH finds nothing for this long, retreat 20 cm "backwards" along the
# CCW arc around the arena centre (i.e. step in the CW tangent direction).
SEARCH_TIMEOUT_S    = 20.0
SEARCH_BACKUP_DIST  = 0.20
ARENA_CENTER_X      = 0.5 * (ARENA_X_MIN + ARENA_X_MAX)
ARENA_CENTER_Y      = 0.5 * (ARENA_Y_MIN + ARENA_Y_MAX)
            ...
            while not self._stop:
                ...
                if mission_state == SEARCH:
                    ...
                    if status == 'commit_to_pass':
                        ...
                        print(f'[{DETECT_1}]')
                    elif time.time() - self._search_start_time >= SEARCH_TIMEOUT_S:
                        # Retreat 20 cm "backwards" along CCW arc around arena
                        # centre (i.e. step along the CW tangent of the circle
                        # through the drone's current position).
                        rx = s['x'] - ARENA_CENTER_X
                        ry = s['y'] - ARENA_CENTER_Y
                        rn = math.hypot(rx, ry)
                        if rn < 1e-3:
                            # At the centre: just back away from current yaw.
                            yaw_r = math.radians(s['yaw'])
                            tx, ty = -math.cos(yaw_r), -math.sin(yaw_r)
                        else:
                            # CW tangent = (ry, -rx) / |r|
                            tx, ty = ry / rn, -rx / rn
                        nx, ny = _clamp_to_arena(s['x'] + SEARCH_BACKUP_DIST * tx,
                                                 s['y'] + SEARCH_BACKUP_DIST * ty)
                        self._hold_x, self._hold_y = nx, ny
                        self._search_start_time = time.time()
                        print(f'[SEARCH] no gate for {SEARCH_TIMEOUT_S:.0f}s → '
                              f'backing up CCW-arc to ({nx:.2f},{ny:.2f})')
