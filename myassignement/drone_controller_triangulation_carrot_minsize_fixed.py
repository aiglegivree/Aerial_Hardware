...
RECT_MIN_AREA_FRAC      = 0.10  # quad area must be ≥ this fraction of the image
...
def _quad_area(quad_pix):
    """Shoelace area of a 4-corner polygon in pixel coords."""
    x = quad_pix[:, 0]
    y = quad_pix[:, 1]
...
def _is_rectangular(quad_pix):
    """True if quad_pix looks roughly rectangular AND fills enough of the
    frame (gate-like). Rejects degenerate, sliver, very skewed, or far-away
    quads. Corners assumed ordered TL → TR → BR → BL by
    cv_detection.order_corners."""
    ...
    if aspect < RECT_MIN_ASPECT:
        return False
    # Discard gates that are too far away to triangulate cleanly: the quad
    # must cover at least RECT_MIN_AREA_FRAC of the full image area.
    if _quad_area(quad_pix) < RECT_MIN_AREA_FRAC * (CAM_WIDTH * CAM_HEIGHT):
        return False
    return True
...
class GateController:
    ...
    def _triangulate(self):
        ...
        v_width = corners_3d[1] - corners_3d[0]   # TL → TR (gate width vector in world)
        # Gate normal (flythrough direction) is perpendicular to v_width in
        # the horizontal plane. _set_gate_waypoints flips by π so it points
        # toward the drone's side, so either perpendicular direction is fine.
        gate_yaw = math.atan2(v_width[0], -v_width[1])
        ...
        try:
            while not self._stop:
                ...
                if mission_state == SEARCH:
                    self._search_yaw_deg = _wrap_yaw_deg(
                    ...
                    if status == 'commit_to_pass':
                        ...
                        self._hold_yaw = _wrap_yaw_deg(
                            math.degrees(math.radians(s['yaw']) - gate_angle))
                        self._frames_detected = 0
                        self._lost_count      = 0
                ...
                # ── TRAVEL_GATE ──────────────────────────────────────────────
                    ...
                    def _carrot_to(tx, ty, tz):
                        ...
                        ahead of the drone toward (tx, ty, tz) in 3-D.
                        Returns the remaining 3-D distance so phase advance
                        also waits for altitude to be reached."""
                        dx = tx - s['x']
                        dy = ty - s['y']
                        dz = tz - s['z']
                    ...
                    if self._travel_phase == 0:
                        ...
                        if dist < PASS_TOLERANCE:
                            ...
                            self._hold_yaw = _wrap_yaw_deg(s['yaw'])
                            self._search_yaw_deg = self._hold_yaw
