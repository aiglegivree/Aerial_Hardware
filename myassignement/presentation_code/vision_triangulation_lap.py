# -*- coding: utf-8 -*-
"""
Part 3: vision-only lap using centre triangulation + carrot sweep.

Gate poses are NOT known in advance. For each gate the drone:
  SEARCH       — rotate in place until the AI-deck camera sees a gate.
  DETECT_1     — settle, record observation 1 (position + centre ray).
  LATERAL_MOVE — carrot to a side baseline; direction chosen from cx bearing.
  DETECT_2     — settle, record observation 2.
  TRIANGULATE  — closest-point intersection of the two centre rays → gate centre.
  TRAVEL_GATE  — carrot through APPROACH → MID → EXIT waypoints.

Lighthouse pose is used for the EKF state estimate (rays are cast in the
world frame) and for arena bound clamping. Gate detection comes from the
camera via `cv_detection2.detect_gate`.
...
# Normalised-x threshold on the gate centre: above this, the gate is on the right.
CX_RIGHT_BIAS    = 0.33

# ── triangulation guards (centre-only) ─────────────────────────────────────────

TRIANGULATE_SKEW_MAX = 0.30   # m
GATE_RANGE_MIN       = 0.2    # m
GATE_RANGE_MAX       = 6.0    # m

# Single-ray triangulation underestimates Z; lift the waypoint.
...
# Camera (x right, y down, z forward) -> body (x forward, y left, z up).
...
class GateController:
    ...
    def __init__(self, cf: Crazyflie, cam: 'UdpVideoThread' = None):
        ...
        # Two views: P/Q observation points, r/s centre rays, qn1/qn2 quads.
        self._P = None
        self._r_centre = None
        self._Q = None
        self._s_centre = None
    ...
    def _centre_ray(self, quad_norm, s):
        """Unit ray (world frame) from the camera through the gate centre."""
    ...
    def _triangulate(self):
        ...
        # Least-squares solve of P + lambda*r = Q + mu*s.
        ...
        # Closest points on each ray; midpoint = estimate, distance = skew.
        F = self._P + lmbda * r
        G = self._Q + mu    * s_dir
        H = (F + G) / 2.0
    ...
    def _check_same_gate(self):
        """Reject if the two observed gate sizes differ too much."""
    ...
    def _set_gate_waypoints(self, H, s):
        """Set approach/mid/exit waypoints; approach axis = gate -> observation midpoint."""
        mid = 0.5 * (self._P + self._Q)
        dx = mid[0] - H[0]
        dy = mid[1] - H[1]
    ...
    def _force_pass(self, s):
        """Skip triangulation and push straight forward along current heading."""
    ...
    def run_mission(self):
        # Reset EKF so world frame aligns with current pose.
                    ...
                    if status == 'commit_to_pass':
                        ...
                        if speed < SPEED_THRESHOLD:
                            ...
                            if self._frames_detected >= REQ_FRAMES:
                                ...
                                # Baseline opposite the bearing for a wider ray crossing.
                                yaw_r = math.radians(s['yaw'])
...
def emergency_stop_listener(ctrl: GateController):
    """'q' cuts motors immediately. ESC requests a controlled land."""
