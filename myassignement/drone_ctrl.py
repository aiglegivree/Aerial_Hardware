...
# ── camera intrinsics & gate geometry (world-frame detection) ──────────────────
# detect_gate_world() turns a pixel detection into a real-world gate position.
# It needs the camera focal length: CAM_FOV_H is a PLACEHOLDER — calibrate the
# AI-deck camera with a checkerboard for accurate coordinates. Until then the
# world coordinates are only approximate (right direction, rough distance).
CAM_FOV_H       = 0.90   # rad — horizontal field of view (TODO: CALIBRATE!)
CAM_FOCAL_PX    = (CAM_WIDTH / 2.0) / math.tan(CAM_FOV_H / 2.0)
GATE_SIZE_M     = 0.40   # m — real gate edge length (TODO: set to your gates)
GATE_REACH_DIST = 0.60   # m — horizontal distance at which APPROACH → TRANSIT
...
class GateController:
    ...
    def detect_gate_world(self):
        """
        Detect a gate and return its CENTRE in the Lighthouse WORLD frame as
        (X, Y, Z) metres, or None if no gate is in view.

        Pinhole model: a gate whose real edge is GATE_SIZE_M, seen `size` px
        across, sits at distance  d = CAM_FOCAL_PX * GATE_SIZE_M / size. The
        pixel offset of the gate centre gives its lateral/vertical position in
        the camera frame; that is rotated by the drone yaw and added to the
        drone position to get world coordinates.

        Accuracy depends on CAM_FOV_H being calibrated and GATE_SIZE_M correct.
        """
        cx, cy, size = get_gate_detection(self._cam.latest_frame)
        if cx is None or size <= 0:
            return None

        # Distance to the gate from its apparent size
        d = CAM_FOCAL_PX * GATE_SIZE_M / size

        # Pixel offsets from the image centre, projected to metres at distance d
        off_right = (cx - CAM_WIDTH  / 2.0) * d / CAM_FOCAL_PX
        off_down  = (cy - CAM_HEIGHT / 2.0) * d / CAM_FOCAL_PX

        # Gate in the BODY frame: x forward, y left, z up
        fwd, left, up = d, -off_right, -off_down

        # Body → world: rotate by drone yaw, translate by drone position
        yaw = math.radians(self._state['yaw'])
        gx = self._state['x'] + math.cos(yaw) * fwd - math.sin(yaw) * left
        gy = self._state['y'] + math.sin(yaw) * fwd + math.cos(yaw) * left
        gz = self._state['z'] + up
        return (gx, gy, gz)

    def approach_gate(self):
        """
        Advance toward the gate using its estimated WORLD position
        (detect_gate_world) — the drone explicitly knows where the gate is and
        navigates to it with boundary-safe hover setpoints, re-estimating the
        position every cycle and yawing to keep the gate in view.

        Returns True  when within GATE_REACH_DIST of the gate (→ TRANSIT).
        Returns False when the gate stays lost beyond LOST_TOLERANCE (→ SEARCH).
        """
        print('  [APPROACH] advancing to gate world position')
        last_world = None
        lost = 0
        tick = 0

        while not self._stop:
            world = self.detect_gate_world()
            if world is not None:
                last_world = world
                lost = 0
            else:
                lost += 1
                if lost > LOST_TOLERANCE or last_world is None:
                    print('  [APPROACH] gate lost — back to SEARCH')
                    return False
                world = last_world  # keep heading to the last known position
            ...
            if tick % 20 == 0:
                print(f'  [APPROACH] gate world ({gx:.2f}, {gy:.2f}, {gz:.2f})  '
                      f'dist={dist:.2f} m')
            tick += 1

            if dist < GATE_REACH_DIST:
                print('  [APPROACH] reached gate vicinity → TRANSIT')
                return True

            # World-frame velocity toward the gate, rotated into the body frame
            ...
            # Yaw to face the gate so the camera keeps it framed
            ...
            # Track the gate's estimated height
            z_cmd = clamp(gz, CRUISE_ALT - 0.4, CRUISE_ALT + 0.4)

            self._safe_hover(vx=bvx, vy=bvy, yaw_rate=yaw_rate, z=z_cmd)
            time.sleep(0.05)
