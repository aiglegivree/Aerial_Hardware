...
Detection pipeline (all on host PC):
  get_gate_mask()        → HSV threshold + morphology → binary mask
  find_gate_rectangles() → flood-fill + minAreaRect   → gate dicts
  get_gate_detection()   → picks closest gate → (cx, cy, size)

Control (visual servoing — no world-frame gate position needed):
...
Boundary safety:
  ...
  Outward velocity linearly suppressed between 1.7 m and 2.0 m.

State machine per gate:  SEARCH → APPROACH → TRANSIT → (next gate)
